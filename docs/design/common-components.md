...
[architecture/overview.md](../architecture/overview.md#app-レイアウト構成)で定義した、全画面共通のヘッダー・下部ナビゲーション・FABをまとめたコンポーネントカタログ。各機能画面（ホーム・取引記録・カテゴリ管理・家族構成管理・AIアドバイス）は、ここで確定したパーツに合わせて実装する。
...
| ②   | 下部固定ナビゲーション       | 5項目（ホーム・取引記録・カテゴリ管理・家族構成管理・AIアドバイス）。アイコン+ラベルを常時表示し、アクティブ項目のみ背景ハイライトで強調（2026-07-25、「非アクティブはアイコンのみ」から変更。詳細は[採用した方向性](#採用した方向性)参照） | 全画面共通                                                 |
...
- **ヘッダー**: 画面タイトル + ユーザーアバターのみのシンプルな構成（[architecture/overview.md](../architecture/overview.md#app-レイアウト構成)の「ヘッダーは最小限」方針通り）。**通知アイコンは含めない**。背景はプライマリグリーンの塗りつぶしではなく、**ページ背景と同じ薄いミントグリーン（`--background-start`）+ 下部の細い罫線**（採番済みスクリーンショット①番で確認。2026-07-23確認）
- **下部固定ナビゲーション**: 5項目（ホーム・取引記録・カテゴリ管理・家族構成管理・AIアドバイス）。アイコン+ラベルは非アクティブ項目でも常時表示し、アクティブ項目のみ背景ハイライトで強調する（2026-07-25決定。モバイルの下部タブバーはiOS Human Interface Guidelines・Material Designともに5項目以下ならラベル常時表示が定番であり、アイコン単体だと意味が伝わりにくいため。当初のPC版カタログ仕様「非アクティブはアイコンのみ」は撤回）。配色は実装時にPC版モックアップ（濃緑ピル+白文字）とSP版個別画面（薄緑丸+濃緑文字）で方針が食い違っていることが判明し、どちらか一方をそのまま踏襲せず、既存トークンの`primary-300`（背景）+白文字（コントラスト比約7.8:1でAAA基準を満たす）に着地した。ホバー時のみ`--color-nav-hover-bg`（`#b6eccd`、SP版準拠の新規トークン）を使う。**採番済みスクリーンショット・PC版モックアップ自体は上記変更前の見た目のまま未再生成（`本格的アドバイス`ラベル・非アクティブアイコンのみ表示を含む）**。5項目それぞれがアクティブな状態を1枚に並べて確認できるようにした
...
- **タブ・セグメントコントロール**: 下線型タブ（アクティブ項目の下に太い緑の下線、非アクティブはグレーテキスト）に統一する。取引記録の「一覧/定期取引」、取引登録フォームの「単発/定期」等、画面内で状態が切り替わるタブ・ラジオ切替はすべてこのスタイルに統一する
  - **例外（2026-07-25決定）**: カテゴリ管理画面のように親タブ（画面遷移）と子サブタブ（絞り込み）が2段で並ぶ場合は、SP版のみ子サブタブをピル型セグメントコントロールにして役割の違いを視覚的に区別する。詳細は[categories/list.mdの採用した方向性](./categories/list.md#採用した方向性)を参照
- **メンバーチップ**: 「本人」「パパ」「ママ」「子ども」を表すラベルチップ。選択中はプライマリグリーンの塗り、非選択中はグレーアウトラインのピル形状で統一する

## 既存実装との差分

未解消の差分なし（2026-07-23発見のヘッダー背景色`bg-primary-500`未定義トークン問題は解消済み。[Header.tsx](../../apps/web/components/Header.tsx)参照）。
...
| 2026-07-25          | 下部固定ナビゲーションの実装方針を変更（非アクティブもアイコン+ラベル常時表示、アクティブのみ背景ハイライト）。ナビラベル「本格的アドバイス」→「AIアドバイス」（[style-guide.mdの用語ルール](./style-guide.md#用語命名の固定ルール)参照）。ヘッダー背景色の未定義トークン問題（`bg-primary-500`）が解消済みであることを確認。モックアップ自体は未再生成 |
