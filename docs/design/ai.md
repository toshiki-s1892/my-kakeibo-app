...
**2026-07-25、ナビラベルを「本格的アドバイス」から「AIアドバイス」に変更**（[style-guide.mdの用語ルール](./style-guide.md#用語命名の固定ルール)参照）。機能名・`DETAILED_ADVICE`コードは変更なし。以下の採番済みスクリーンショット・Stitch Screen IDのタイトルは変更前のもので、画面内には旧ラベル「本格的アドバイス」がそのまま写っている（未再生成）。

## 関連画面

| 遷移元                                                           | 遷移先                                                                   |
| ---------------------------------------------------------------- | ------------------------------------------------------------------------ |
| 下部固定ナビゲーション（どこからでも）                           | `(app)/advice`（「AIアドバイス」タブ、トピック選択前の初期状態）         |
...
- **ナビゲーション**: 他画面と共通の下部固定タブバー（5項目、AIアドバイスがアクティブ）+「+ 取引を追加」FAB
...
| 2026-07-25 | ナビラベルを「本格的アドバイス」から「AIアドバイス」に変更（機能名・`DETAILED_ADVICE`は維持）。採番済みスクリーンショット・Stitch Screen IDのタイトルは旧ラベルのまま未再生成                                                                                                                                                           |
