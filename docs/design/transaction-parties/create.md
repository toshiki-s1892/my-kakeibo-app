...
詳細は[機能仕様](../../specs/features/transaction-parties.md#apiエンドポイント)を参照。~~`typeCode`は[transaction-parties/list.md](./list.md)で選択中の支出/収入サブタブから決まるため、このDialog自体には入力欄を設けない。~~ この記述は[今後反映予定の変更](#今後反映予定の変更モックアップ未更新)で上書きされた（カテゴリ新規追加Dialogと共通化し、`typeCode`もDialog内で選択させる方針に変更）。
...
- **カテゴリ新規追加Dialogとの共通化（2026-07-25決定）**: 名前のみの単独Dialogから、[categories/create.md](../categories/create.md)のカテゴリ新規追加Dialogと共通化した1つのフォームに変更する。フォーム内で対象（カテゴリ/取引先）を切り替え可能にし、切り替えに応じて呼び出すAPI（`POST /api/categories` / `POST /api/transaction-parties`）を変える
- **`typeCode`（支出/収入）の入力方法変更**: 「一覧のサブタブから継承し、Dialogには入力欄を設けない」という上記の方針をやめ、支出/収入も初期値なしでDialog内で毎回選択させる（対象をカテゴリ/取引先どちらに切り替えても共通）
- 現行モックアップ（名前のみの単独Dialog）はこの変更前の方針のまま生成されているため、再生成が必要（[tasks/categories.md](../../tasks/features/categories.md)参照）
...
| 2026-07-25 | 実装検討の中で、カテゴリ新規追加Dialogとの共通化・`typeCode`のDialog内選択への変更を決定。[今後反映予定の変更](#今後反映予定の変更モックアップ未更新)に記録（モックアップ再生成は別タスク） |
