...
| ⑨   | 下部固定ナビゲーション     | 5項目（ホーム・取引記録・カテゴリ管理・家族構成管理・AIアドバイス）。「ホーム」がアクティブ                                                                               | 各タブへ遷移                                                                                                        |
...
| ローディング状態       | [frontend-conventions.md](../architecture/decisions/frontend-conventions.md#フロントエンドのエラーハンドリング方針)を参照。初回取得中はサマリー・グラフ部分をスピナー表示                  |
