...
[機能仕様](../specs/features/transaction-parties.md)に対応する取引先削除確認AlertDialog。[transaction-parties-list.md](./transaction-parties-list.md)の各行の削除アイコンから開く。見た目の共通フレームワークは[modals.md](./modals.md#alertdialog共通構成カテゴリ削除確認家族メンバー削除確認取引削除確認)を参照。

## 関連画面

| 遷移元 | 遷移先 |
|---|---|
| [transaction-parties-list.md](./transaction-parties-list.md)各行の削除アイコン | 取引先削除確認AlertDialog（同画面上にAlertDialog表示） |
...
- 背景に表示されている下層画面（「よく使う取引先」のカード一覧UI）は、[transaction-parties-list.md](./transaction-parties-list.md)で確定したフラットな1列リストとは異なる構成。Stitchが生成時に参照した別バージョンであり、実装時の背景画面は[transaction-parties-list.md](./transaction-parties-list.md)の確定モックアップを参照すること
