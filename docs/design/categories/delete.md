...
[機能仕様](../../specs/features/categories.md)に対応するカテゴリ削除確認AlertDialog。[categories/list.md](./list.md)の各行の削除アイコン（独自カテゴリのみ）から開く。見た目の共通フレームワークは[modals.md](../modals.md#alertdialog共通構成カテゴリ削除確認家族メンバー削除確認取引削除確認)を参照。

## 関連画面

| 遷移元                                                                | 遷移先                                                   |
| --------------------------------------------------------------------- | -------------------------------------------------------- |
| [categories/list.md](./list.md)各行の削除アイコン（独自カテゴリのみ） | カテゴリ削除確認AlertDialog（同画面上にAlertDialog表示） |
...
- 背景に表示されている下層画面は、Stitchが生成時に参照した旧バージョンであることが多く、実装時の背景画面は[categories/list.md](./list.md)の確定モックアップを参照すること
