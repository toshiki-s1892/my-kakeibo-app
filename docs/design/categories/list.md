...
| ⑤   | カテゴリ行                         | 円形カラーアイコン+カテゴリ名+ピン留めアイコン（線=OFF/塗り=ON。EXPENSE側のみ表示）                                | 行クリックで編集Dialog（[categories/edit.md](./edit.md)）。ピンアイコンクリックでピン留めON/OFF。**仕様変更（編集・削除不可の制約撤廃）により、初期カテゴリの行にも編集・削除アイコンを表示する必要がある。現行モックアップは旧仕様（デフォルトは編集・削除不可）のまま生成されているため再生成が必要**（[仕様外要素](#仕様外要素実装時は無視すること)参照） |
