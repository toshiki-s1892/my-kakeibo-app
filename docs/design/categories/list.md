...
[機能仕様](../../specs/features/categories.md)に対応する画面（`(app)/categories`）の一覧表示部分。新規作成・編集・削除は[categories/create.md](./create.md)・[categories/edit.md](./edit.md)・[categories/delete.md](./delete.md)を参照。
...
| 「新しいカテゴリを追加」ボタン         | カテゴリ新規追加Dialog（[categories/create.md](./create.md)）      |
| 各行の編集アイコン                     | カテゴリ編集Dialog（[categories/edit.md](./edit.md)）              |
| 各行の削除アイコン                     | カテゴリ削除確認AlertDialog（[categories/delete.md](./delete.md)） |
...
| ③   | 支出/収入サブタブ                  | PC版は下線型タブ、SP版はピル型セグメントコントロール（意図的な採用。[採用した方向性](#採用した方向性)参照）                                                                                                                                                                                                                                                                                                                       | タップで表示対象の`typeCode`を切り替え                                                                                                                                                                                                                                                                                                                       |
| ④   | 「新しいカテゴリを追加」ボタン     | PC版は画面右上、SP版はタブ下に幅いっぱい                                                                                                                                                                                                                                                                                                                                                                                          | カテゴリ新規追加Dialogを開く（[categories/create.md](./create.md)参照）                                                                                                                                                                                                                                                                                      |
| ⑤   | カテゴリ行                         | 円形カラーアイコン+カテゴリ名+ピン留めアイコン（線=OFF/塗り=ON。EXPENSE側かつ親・単独カテゴリのみ表示）                                                                                                                                                                                                                                                                                                                           | 行クリックで編集Dialog（[categories/edit.md](./edit.md)）。ピンアイコンクリックでピン留めON/OFF。**仕様変更（編集・削除不可の制約撤廃）により、初期カテゴリの行にも編集・削除アイコンを表示する必要がある。現行モックアップは旧仕様（デフォルトは編集・削除不可）のまま生成されているため再生成が必要**（[仕様外要素](#仕様外要素実装時は無視すること)参照） |
| ⑥   | 子カテゴリの開閉トグル             | 親カテゴリ（光熱費）の行に表示されるシェブロンアイコン。初期状態は展開済み                                                                                                                                                                                                                                                                                                                                                        | タップで子カテゴリ（電気代・ガス代・水道代）の表示/非表示を切り替え                                                                                                                                                                                                                                                                                          |
| ⑦   | 親子カテゴリのグルーピング         | 光熱費の下にツリー線付きインデントで子カテゴリ（電気代・ガス代・水道代）を表示。**子カテゴリ行も親と同じ円形カラーアイコン+編集アイコンを表示するが、ピン留めアイコンは表示しない**（子カテゴリはピン留め不可のため。[ピン留め](../../specs/features/categories.md#カテゴリの固定表示ピン留め)参照）。現行モックアップは子カテゴリのアイコン・色バッジが未生成のため再生成が必要（⑤の編集・削除アイコン再生成要件とあわせて対応） | 子カテゴリも自分のものであれば編集可能（ピン留めは不可。[親カテゴリ](../../specs/features/categories.md#親カテゴリグラフ上のグルーピング)参照）                                                                                                                                                                                                              |
| ⑧   | 編集・削除アイコン（独自カテゴリ） | 自分が追加したカテゴリ（サブスク代・ペット費等）の行にのみ表示するペンシル・ゴミ箱アイコン                                                                                                                                                                                                                                                                                                                                        | 編集アイコンで[categories/edit.md](./edit.md)、削除アイコンで[categories/delete.md](./delete.md)                                                                                                                                                                                                                                                             |
...
| 状態                       | 表示内容                                                                                                                                                                                                                                                        |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 空状態                     | 基本的に発生しない。プロフィールセットアップ完了時に初期カテゴリ（支出12種・収入4種）が自動作成されるため。ただしユーザーが初期カテゴリを削除し尽くした場合は空になり得る（EXPENSE側は最低1件ピン留めが必須のためゼロにはならないが、INCOME側はゼロになり得る） |
| エラー状態                 | [frontend-conventions.mdのエラーハンドリング方針](../../architecture/decisions/frontend-conventions.md#フロントエンドのエラーハンドリング方針)を参照。一覧の初回取得失敗はコンテンツ差し替え+再試行（モックアップ上の表現はなし）                               |
| ローディング状態           | [frontend-conventions.md](../../architecture/decisions/frontend-conventions.md#フロントエンドのエラーハンドリング方針)を参照。初回取得中はリスト部分をスケルトン表示（モックアップ上の表現はなし）                                                              |
...
- 支出/収入サブタブのスタイルがPC版は下線型、SP版はピル型と異なる（[採用した方向性](#採用した方向性)参照。意図的な差分であり統一しない）
...
- **（旧方針・撤廃済み）デフォルトカテゴリの編集・削除アイコン非表示**: 以前は[権限ルール](../../specs/features/categories.md#権限ルール)上システムデフォルトは編集・削除不可としていたため、デフォルトカテゴリの行にはピン留めアイコンのみを表示する指示をしていた。その後「`userId`を必須にし、全カテゴリをユーザー所有にする」設計に変更したため、初期カテゴリも編集・削除アイコンを表示する必要がある。現行モックアップ・スクリーンショットは旧方針のまま生成されているため、再生成時はこの制約を外した指示に変更すること
...
- **支出/収入サブタブのPC/SPスタイル差分（2026-07-25決定）**: 「カテゴリ/取引先」親タブ（画面遷移）と支出/収入サブタブ（`typeCode`の絞り込み）が縦に2段並ぶSP版では、同じ見た目のタブが連続すると役割の違いが伝わりにくい。親タブは下線型のまま、子サブタブのみPC版は下線型・SP版はピル型セグメントコントロールと形状を変えることで、「上は画面遷移、下はその場でのフィルタ」という階層を視覚的に示す。PC版は余白に余裕がありタブ間の階層が伝わりやすいため両方下線型で統一する。実装はTailwindの`md:`によるCSSのみで出し分け、`useMediaQuery`は使わない（詳細は[frontend-conventions.mdのレスポンシブ対応](../../architecture/decisions/frontend-conventions.md#レスポンシブ対応-usemediaquery-パターン)参照）
...
| 2026-06-22（4回目） | 一覧・新規作成・編集・削除が1ファイルに混在し読みづらいとのユーザー指摘を受け、`categories.md`を分割。本ファイルは一覧部分のみを担当（[categories/create.md](./create.md)・[categories/edit.md](./edit.md)・[categories/delete.md](./delete.md)に分割）                                                                                                         |
...
| 2026-07-25          | 実装検討の中で、支出/収入サブタブのPC下線型/SP ピル型の差分を「仕様外要素（要統一）」から「意図的な採用」に方針転換。親タブ（画面遷移）と子サブタブ（絞り込み）の役割を視覚的に区別する目的と判明したため、`common-components.md`のタブ統一方針の例外として明記                                                                                                 |
| 2026-08-01          | `/grill-me`セッションで、GET `/api/categories`のレスポンス構造（ネスト`children`）に合わせて仕様を精査。子カテゴリはピン留め不可と決定（[categories.mdのピン留め節](../../specs/features/categories.md#カテゴリの固定表示ピン留め)参照）、子カテゴリ行も親と同じアイコン+色バッジを表示する方針に変更。パーツ⑤・⑦を更新し、モックアップ再生成が必要な項目に追加 |
