...
| セットアップ手順・コマンド一覧・開発フローの全体図（どのチェックがどこで実行されるか） | ルートの[README.md](../README.md)（分担の理由・背景は[architecture/decisions/dev-workflow.md](./architecture/decisions/dev-workflow.md)。同じ内容を両方に書かない） |
