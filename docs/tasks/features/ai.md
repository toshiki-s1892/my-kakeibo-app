...
- [ ] 6. `server/routes/dashboard/handler.ts`（簡易支出分析）: 当日の`ai_usage_logs`（featureCode=2）の有無を確認し、無ければ生成・`content`に保存。1日1回のキャッシュ。**Geminiへのプロンプト・保存する`content`には集計値（合計・カテゴリ別金額）のみを含め、`family_members.name`は含めない**（[security.mdのプライバシー方針](../../architecture/decisions/security.md)に基づく最小化。2026-08-23決定）
...
- [ ] 3. AIアドバイス画面のトピック選択UIに同様に「本日の残り◯回」を表示
