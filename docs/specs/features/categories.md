...
食費・交通費・住まいの費用・光熱費・通信費・医療費・服・靴・日用品費・趣味・娯楽・教育費・保険料・その他

「住まいの費用」（旧称: 住居費）・「服・靴」（旧称: 被服費）・「趣味・娯楽」（旧称: 娯楽費）は、初見でも迷わない平易な表現にするため命名を見直したもの。「住まいの費用」は賃貸・持ち家のどちらにも当てはまる中立的な表現とし、「家賃」のような賃貸限定の語は避けた。

このうち**住まいの費用・食費・光熱費は、ユーザー作成時に初期状態でピン留め**（[カテゴリの固定表示](#カテゴリの固定表示ピン留め)参照）される。

**IDについて**: デフォルトカテゴリ16件（支出12種・収入4種）の`id`は、他の全テーブルと同じく`crypto.randomUUID()`による自動生成のままとする（固定UUIDのリテラル指定は不要）。プロフィールセットアップ完了時のピン留め処理（[カテゴリの固定表示](#カテゴリの固定表示ピン留め)参照）が「住まいの費用・食費・光熱費」を特定する必要があるが、これは`categories.isDefaultPinTarget`（boolean）フラグで`WHERE userId IS NULL AND isDefaultPinTarget = true`を検索すれば済むため、`id`の値そのものをアプリコードやTS定数に固定で持つ必要がない。

**検討した代替案（不採用）:** (1) 固定UUIDをseedとTS定数の両方にリテラルで持つ方式は、36文字のUUID文字列を2箇所で完全一致させる必要があり、1文字の違いに気づきにくいため不採用。(2) `code`のような業務的な識別子カラムを追加し名前解決する方式も検討したが、現状「ピン留め対象の集合を取得する」以外に個別の特定カテゴリを参照する要求が無いため、boolean一つで十分な現状ではYAGNI（過剰設計）と判断し不採用。将来「特定の1カテゴリを名前で正確に指したい」という要求が出た場合は、表示用の`name`に依存せず別の識別子カラムを検討する。

光熱費は電気代・ガス代・水道代等に分けず、デフォルトでは1カテゴリのままとする（[親カテゴリ機能](#親カテゴリグラフ上のグルーピング)の説明例として使われているため、デフォルトは粗い粒度にとどめ、細分化したい家庭は親カテゴリ機能で自分で分割する想定）。
...
- システムデフォルトカテゴリのうち**住まいの費用・食費・光熱費は、ユーザー作成時（プロフィール設定完了時）にそのユーザー向けの`category_pins`が3件INSERTされ、初期状態でピン留み済みになる**。それ以外はユーザーが必要に応じて設定する。対象3件は`categories.isDefaultPinTarget = true`で検索して特定する（[デフォルトカテゴリのIDについて](#デフォルトカテゴリuserid--null)参照）
- ピン留めが円グラフに与える具体的な効果（金額順位に関わらず必ず表示される、という点）はカテゴリ一覧画面上では明示されないため、[ヘルプページ](./help.md#カテゴリ管理)で説明する
...
モーダル内では図解までは表示しない（スクロールが増えるため）。ヘルプテキストの文末に「詳しくはヘルプページへ」という[ヘルプページ](./help.md#カテゴリ管理)の該当セクション（`/help#categories`）へのリンクを置き、図解（3つの子カテゴリが1つの親カテゴリに合算される様子を示すミニ図解）はそちらに掲載する。
...
| 名前の重複 | 同一タイプ内で、システムデフォルト + 自分のカテゴリを含めて重複不可（編集時は自分自身を除外して判定）。アプリ層での判定に加え、DB側にも`(userId, typeCode, name)`の部分一意制約（`deletedAt IS NULL`の行のみ対象）を defense-in-depth として設定する（システムデフォルトとの重複は`userId`が異なるため制約ではカバーできず、アプリ層判定が必須。詳細は[architecture/database.md](../../architecture/database.md)参照） |
