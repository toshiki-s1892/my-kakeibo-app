...
**画像サイズ:** Vercelはリクエストボディを4.5MBに強制上限しており（[api-conventions.mdのリクエストボディサイズの上限](../../architecture/decisions/api-conventions.md#リクエストボディサイズの上限2026-08-23決定)参照）、スマートフォンで撮影した写真はこれを容易に超える（5〜10MBになることも多い）。**送信前にクライアント側で圧縮・リサイズしてから送る**（2026-08-23決定。具体的な圧縮ライブラリ・目標サイズは実装時に決定）。
...
- Geminiへのプロンプトに含めるデータは、[プライバシー方針](../../architecture/decisions/security.md)に基づき最小化する（2026-08-23決定）

  | 含める                                                           | 含めない                                                                             |
  | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
  | 取引データ: カテゴリ名・金額・日付                               | 取引先名（`transaction_parties.name`。勤務先・店舗名等が第三者提供に該当し得るため） |
  | 居住地域: `regionCode`（都道府県コード）                         | 具体的な住所                                                                         |
...
`RECEIPT_SCAN`・`DETAILED_ADVICE`はそれぞれ独立した1日の上限を持つ（[1](#1-レシート読み取り自動入力receipt_scan)・[3](#3-本格的アドバイスdetailed_advice)参照。`ai_usage_logs.featureCode`で完全に独立してカウントされるため、片方を使い切っても他方には影響しない）。上限に達するまで残り回数を確認する手段がなかったため、利用場所ごとに残り回数を表示する。

- レシート読み取りUI（取引登録フォームの「レシートから読み取る」ボタン付近）に「本日の残り◯回」を表示する
- 本格的アドバイス画面のトピック選択UIに「本日の残り◯回」を表示する
- ダッシュボードやヘルプページには表示しない（実際に使おうとしているその場で見える方が実用的という判断。[ヘルプページ](./help.md)には静的な上限値の説明のみ記載する想定）

**API:** `GET /api/ai/usage?featureCode=`（`RECEIPT_SCAN`・`DETAILED_ADVICE`共通。当日の`ai_usage_logs`件数から残り回数を計算して返す。2機能で同じ集計ロジックを再利用する）
