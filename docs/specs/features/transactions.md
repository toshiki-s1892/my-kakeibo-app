...
編集時は、現在選択されている家族メンバー・カテゴリ・取引先が論理削除済みであっても、選択肢としてそのまま表示する（既存の選択を維持するため）。同名の項目が新規作成されている場合に区別できるよう、削除済みの選択肢はグレー文字＋「(削除済み)」ラベルで表示する（詳細は[design/style-guide.md](../../design/style-guide.md#論理削除済み項目の表示)参照）。
